 ...
 */
const PLATE_METADATA: Record<PlateId, PlateMetadata> = {
  I: {
    id: 'I',
    name: 'Golden Ratio',
 ...
 */
const NONOGRAM_TRANSITION_GRAPH: NonogramTransitionGraph = {
  edges: [
 ...
 */
export class NonogramCodex {
  private config: CodexConfig;
  private state: CodexState;
  private currentCycle: CycleRecord | null = null;
  private cycleHistory: CycleRecord[] = [];
  private signalHistory: SignalAlignment[] = [];
   ...
   */
  private initializeState(): CodexState {
    return {
   ...
   */
  private getNextPlate(fromPlate: PlateId): PlateId | null {
   ...
   */
  private getOperatorForPlate(plateId: PlateId): OperatorName {
   ...
   */
  alignSignalToPlate(signal: MarketSignal): SignalAlignment {
    let plateId: PlateId = 'I'; // default
    let score = 0.5;
   ...
   */
  async step(signal?: MarketSignal): Promise<PlateRecord> {
    ...
    let alignment: SignalAlignment | undefined;
    if (signal) {
   ...
   */
  async runCycle(maxSteps = 10): Promise<CycleRecord> {
   ...
   */
  setFallbackMode(active: boolean, reason?: string): void {
   ...
   */
  getRecentCycles(count = 5): CycleRecord[] {
   ...
   */
  getSignalHistory(): SignalAlignment[] {
   ...
   */
  getRecentSignals(count = 10): SignalAlignment[] {
   ...
   */
  getTransitionGraph(): NonogramTransitionGraph {
   ...
   */
  getPlateMetadata(): Record<PlateId, PlateMetadata> {
  ...
  verifyInvariants(): { valid: boolean; violations: string[] } {
    ...
    if (this.state.metadata.transitionsCorrect !== this.state.metadata.transitionsTotal) {
      violations.push(
        `Incorrect transitions: ${this.state.metadata.transitionsCorrect}/${this.state.metadata.transitionsTotal}`
      );
    }
 ...
 */
export function createNonogramCodex(config?: Partial<CodexConfig>): NonogramCodex {
  const finalConfig: CodexConfig = {
    initialPlate: 'I',
    sealed: true,
