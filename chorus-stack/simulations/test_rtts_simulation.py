...
class TestRunScenario(unittest.TestCase):
    ...
    def test_measured_contamination_clipped_to_zero_when_survivor_bias_exceeds_contamination(self):
        # survivor_bias > contamination gives negative measured; should be clipped to 0
    ...
    def test_containment_not_triggered_at_exactly_0_40_confidence_drift(self):
        # confidence_drift = exactly 0.40: load=400, measured=0 (contamination <= survivor_bias)
        # drift = 0 * 1.2 + 400/1000 = 0.4
    ...
    def test_outcome_is_correct_dataclass_type(self):
        s = Scenario("check-type", contamination=0.1, load=50, survivor_bias=0.0)
        result = run_scenario(s)
        self.assertIsInstance(result, Outcome)

    def test_survivor_bias_equal_to_contamination_gives_zero_measured(self):
        s = Scenario("zero-measured", contamination=0.25, load=100, survivor_bias=0.25)
        result = run_scenario(s)
        self.assertAlmostEqual(result.measured_contamination, 0.0)

    def test_negative_survivor_bias_amplifies_measured_contamination(self):
        # survivor_bias = -0.1 effectively adds 0.1 to contamination
        s_no_bias = Scenario("no-bias", contamination=0.2, load=0, survivor_bias=0.0)
        s_neg_bias = Scenario("neg-bias", contamination=0.2, load=0, survivor_bias=-0.1)
        no_bias_result = run_scenario(s_no_bias)
        neg_bias_result = run_scenario(s_neg_bias)
        self.assertGreater(
            neg_bias_result.measured_contamination,
            no_bias_result.measured_contamination
        )


class TestSimulate(unittest.TestCase):
    def test_simulate_returns_correct_number_of_outcomes(self):
        scenarios = [
            Scenario("s1", contamination=0.05, load=50, survivor_bias=0.0),
            Scenario("s2", contamination=0.15, load=100, survivor_bias=0.0),
            Scenario("s3", contamination=0.35, load=200, survivor_bias=0.0),
        ]
        outcomes = simulate(scenarios)
        self.assertEqual(len(outcomes), 3)

    def test_simulate_preserves_order_of_scenarios(self):
        scenarios = [
            Scenario("first", contamination=0.05, load=10, survivor_bias=0.0),
            Scenario("second", contamination=0.1, load=20, survivor_bias=0.0),
        ]
        outcomes = simulate(scenarios)
        self.assertEqual(outcomes[0].name, "first")
        self.assertEqual(outcomes[1].name, "second")

    def test_simulate_returns_empty_list_for_empty_input(self):
        outcomes = simulate([])
    ...
    def test_simulate_matches_individual_run_scenario_calls(self):
        ...
        bulk = simulate(scenarios)
        for i, s in enumerate(scenarios):
            individual = run_scenario(s)
            self.assertAlmostEqual(bulk[i].measured_contamination, individual.measured_contamination)
            self.assertAlmostEqual(bulk[i].confidence_drift, individual.confidence_drift)
            self.assertEqual(bulk[i].containment_triggered, individual.containment_triggered)

    def test_simulate_accepts_generator_input(self):
        def gen():
            yield Scenario("gen1", contamination=0.05, load=50, survivor_bias=0.0)
            yield Scenario("gen2", contamination=0.2, load=100, survivor_bias=0.0)

        outcomes = simulate(gen())
        self.assertEqual(len(outcomes), 2)
        self.assertEqual(outcomes[0].name, "gen1")
        self.assertEqual(outcomes[1].name, "gen2")
